...
- Chrome Model Context Tool Inspector natural-language selection and permission-transition acceptance has not been run. The recorded Chrome 153 result now proves local browser API discovery, schema inspection, safe manual execution, execution cancellation, `toolchange`, registration abort, and cleanup, but it does not prove agent prompt selection, the Inspector UI, or a production Origin Trial deployment. The separate 55-sample Ollama tool-calling baseline proves selection behavior only and does not replace that gate.
