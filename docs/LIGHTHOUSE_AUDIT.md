...
After adding the fail-closed Origin Trial deployment contract and compact status disclosure, Lighthouse 13.4.1 reran against the isolated production server. The default 375×812 audit recorded Performance 95, Accessibility 100, Best Practices 100, FCP 1.3 s, LCP 2.6 s, TBT 150 ms, CLS 0, zero console errors, and zero non-composited-animation findings. The fragment-targeted 844×390 landscape audit recorded Performance 93, Accessibility 100, Best Practices 100, FCP 1.6 s, LCP 2.7 s, TBT 190 ms, CLS 0, and zero console errors. The desktop audit recorded Performance 100, Accessibility 100, Best Practices 100, FCP 0.4 s, LCP 0.6 s, TBT 0 ms, CLS 0, and zero console errors. SEO remained 66 under the deliberate no-index profile. A fragment-targeted 375×812 visual run retained Accessibility and Best Practices 100 with CLS 0 and zero console errors; its Performance 86/TBT 420 ms is retained as the observed cost of navigating directly into the long evidence page rather than substituted with the faster top-of-page run. Original-size screenshots were inspected: desktop and landscape keep the recorded runtime and Origin Trial rows compact and aligned, while mobile reflows the deployment label, state, and 44px disclosure affordance without clipping before the judge runbook. Several discarded attempts returned Lighthouse `NO_NAVSTART`; accepted reports were parseable before the known Windows temporary-profile cleanup `EPERM`.

After adding Live Agent Rehearsal, Lighthouse 13.4.1 targeted `/webmcp#live-agent-rehearsal-title` on an isolated `next start` server at port 3107. The production report recorded Performance 92, Accessibility 100, Best Practices 100, SEO 66, FCP 1.5 s, LCP 2.8 s, TBT 230 ms, CLS 0, zero console errors, and zero non-composited-animation findings. The audit covered the server-rendered idle state and sent no cloud request. The complete JSON was parsed before Chrome launcher reported the known temporary-profile cleanup `EPERM`; the profile was already absent afterward. The temporary report was removed, the exact port-3107 owning process was stopped, and the user server on port 3001 remained HTTP 200. A separate live fixed-input request exercised the Traditional Chinese search scenario: `gpt-oss:120b-cloud` selected `search_public_cancer_trials` in 1,343 ms, the provider reported `gpt-oss:120b`, and the receipt confirmed no execution, no health information, no model-content storage, and no persistence.

After adding the separately gated fixed public browser-execution panel, the same fragment was rebuilt and audited on isolated production port 3109. The report recorded Performance 91, Accessibility 100, Best Practices 100, FCP 1.688 s, LCP 2.192 s, TBT 310 ms, CLS 0, and zero console errors. This audit covered only the initial no-request state; the execution panel intentionally appears only after a passing live Traditional Chinese selection and therefore is not claimed as Lighthouse interaction evidence. A separate live no-PHI dependency check recorded the cloud model selecting `search_public_cancer_trials` in 698 ms and the fixed `胃癌 → gastric cancer` registry bridge returning ten bounded candidates from two completed sources with zero source failures. Those two checks still do not establish that the combined second-click `document.modelContext.executeTool()` path ran in a compatible Chrome session. The complete Lighthouse JSON was parsed before the known post-report profile-cleanup `EPERM`; the exact port-3109 process was stopped and port 3001 remained HTTP 200.

After upgrading the banner to the recorded stock-Inspector 3/6 result, Lighthouse 13.4.1 reran `/webmcp` on an isolated production server at port 3002. The valid mobile report recorded Performance 85, Accessibility 100, Best Practices 100, CLS 0, zero console errors, and zero non-composited-animation findings. Chrome launcher reported the known cleanup-only `EPERM` after the complete JSON had been written and parsed; its exact temporary path was already absent afterward.

Extension-free production screenshots were inspected at 375px and 844px widths. The three textual Pass cards, one Not run card, provider-policy boundary, and evidence links remained fully visible without horizontal overflow. At 844px the three Pass cards aligned in one equal-height row; at 375px all four cards stacked at one width. The temporary report, screenshots, profiles, and port-3002 process were removed; the user server on port 3001 remained listening.
