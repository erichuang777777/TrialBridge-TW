...
`GET /webmcp/evidence.json` is a force-static public competition artifact with a five-minute cache policy. It contains capability metadata, the four-state synthetic registration model, evidence classes, source paths/links, and selection-artifact digests only. It reads no request, browser session, note, profile, results, or chat; the separate user-downloaded browser diagnostic receipt remains the only current-browser evidence artifact. The state model is unit-tested against runtime tool construction but executes no tool and is not current-browser evidence.
