...
`GET /webmcp/evidence.json` is a force-static public competition artifact with a five-minute cache policy. It contains capability metadata, the four-state synthetic registration model, the six-check runtime-suite definition, evidence classes, source paths/links, and selection-artifact digests only. It reads no request, browser session, note, profile, results, or chat; the separate user-downloaded browser diagnostic receipt remains the only current-browser evidence artifact. The state model and suite definition are repository evidence, not claims that a current browser ran either path.
