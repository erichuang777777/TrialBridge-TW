...
`npm run build`; unchecked items need an external account (Turso, Netlify,
Chrome Origin Trials, the custom domain) or a live measurement.
...
- [ ] After the first Netlify deploy, confirm a 12-second-plus streamed extraction is not cut at 10 seconds. If it is, add Netlify Edge Functions under `netlify/edge-functions/` for the four `/api/cloud/*` paths, importing the same `lib/llm/*` modules.
...
- [ ] Run the three `sqlite3` inventory queries against the local 4 GB index and confirm `fts_schema_version = 3`.
- [ ] `turso group create` (Tokyo), `turso db import` the demo subset, create a read-only token for Netlify and a read-write token for GitHub secrets.
- [ ] Verify FTS5 writes over HTTPS by running `npm run sync:trial-index -- --source=tfda` against Turso (libsql issue #1811 concerned the embedded client; confirm the remote path).
...
- [ ] Add the custom domain in Netlify, enable Force HTTPS, set `SITE_URL` to the exact origin.
- [ ] Set `OLLAMA_API_KEY`, `TRIAL_INDEX_LIBSQL_URL`, `TRIAL_INDEX_LIBSQL_AUTH_TOKEN` (read-only) in the Netlify UI.
...
- [ ] Register a Chrome Origin Trial token for the custom domain; set `WEBMCP_ORIGIN_TRIAL_TOKEN`; confirm `Enabled` in DevTools.
