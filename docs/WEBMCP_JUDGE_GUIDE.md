...
The page also includes a compact ten-row judge conformance matrix. It prevents different evidence types from being blended into one green score: seven claims are repository-verified, the stock Inspector 2/6 result is labelled recorded browser runtime, the selection baseline is labelled recorded model evaluation, and four Inspector cases remain a manual gate. A judge can download `/webmcp/evidence.json` to inspect the same capability inventory, evidence paths, source links, audit metadata, recorded runtime summaries, and full dataset/tool-contract digests. The static bundle contains no current browser or medical-workflow state and is explicitly not a protocol endpoint.
