...
1. Open `/webmcp`. Start with the recorded-browser banner: Chrome for Testing 153.0.8010.12, 6/6 lifecycle checks, zero console errors, probe absent, and the exact receipt link. Run the body-free Demo preflight and confirm cloud, TFDA, and ClinicalTrials.gov readiness or source-level recovery. Then show the current browser-support state, two public tool discovery count, and same-origin security headers. When supported, run **One-click WebMCP lifecycle acceptance**, expand its outcomes, confirm cleanup, and download the no-health-data current-browser receipt. Continue with the critical journey, capability inventory, conformance matrix, 55-sample selection baseline, canonical contracts, and 2 → 2 → 6 → 7 simulator. Keep the recorded runtime, current-browser run, static evidence, cloud-model selection, and self-attested Manual Chrome gate visibly distinct.
