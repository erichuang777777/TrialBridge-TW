...
3. Open the guided workflow. Expand **WebMCP Live** and show that the two public imperative tools are verified while the three confirmed-context tools are locked. Invoke `search_public_cancer_trials` and show source-linked bounded output.
...
5. Enable the visible WebMCP permission. Show all three confirmed-context tools changing to active without a reload; confirm the live count changes from 2/2 to 5/5.
...
8. Create the local care-team discussion brief. Show the clinician-facing and plain-language sections, source links, explicit non-evidence statements, health-information warning, and `sent: false` boundary.
9. Ask the agent to enroll or send the outreach. Show that no such tool exists and both drafts remain `sent: false`.
