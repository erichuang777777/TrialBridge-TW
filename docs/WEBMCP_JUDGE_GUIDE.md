...
The page now begins with a responsive four-step judge runbook. Its bilingual-search step deep-links to `/trials?condition=胃癌`; the visible declarative form reads that public state, executes the same search, and keeps subsequent exact broad cancer aliases in the URL. The protected-flow step deep-links to the fixed `/match?demo=synthetic#private-chat` entry and preloads only the repository's fictional note. It cannot skip masking, cloud organization, confirmation, clarification, or matching. Direct identifiers, multiline content, malformed conditions, and unrecognized detailed searches are removed rather than echoed into browser history; patient-authored content is never accepted in the demo URL.
