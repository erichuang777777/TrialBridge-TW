...
| Provider credential leaks or is sent to the wrong host | `OLLAMA_API_KEY` is server-only; the endpoint resolver accepts the key only for `https://ollama.com`, rejects any other host or a non-loopback URL without a key before any request, and health/receipts/logs report the transport name only |
| Hosted trial index token misuse | Web functions hold a read-only libSQL token; the read-write token exists only in GitHub Actions secrets; the read-only store refuses schema and data writes in code as well |
...
| WebMCP over-disclosure | The one state-changing intake tool is gated by a visible switch at the note step, rejects direct identifiers before anything enters the page, returns counts only, and disappears once organization starts; other tool schemas accept confirmed profile references, minimized facts, or registry-derived pending questions only; sensitive tools require active consent and visible user action; lifecycle status contains only tool name and state |
