...
7. Results are retrieved Taiwan first, then Asia, then worldwide and explain source, freshness, possible mismatches, and unknowns. Each result maps confirmed subtype, stage, biomarker, and prior-treatment facts to traceable public wording as shared term, possible difference, uncertain, or missing; the map never declares eligibility or changes overall status. Public direct search exposes a versioned 19-group bilingual query plan: an exact curated cancer term is sent to TFDA in Traditional Chinese and ClinicalTrials.gov in English; an unrecognized detailed term passes through unchanged without inferred subtype, stage, or biomarker.
