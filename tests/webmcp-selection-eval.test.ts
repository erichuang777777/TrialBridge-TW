...
test("recorded cloud-model baseline is current, internally consistent, and stores no model prose", async () => {
  ...
  assert.equal(baseline.summary.passed, 55);
  assert.equal(baseline.summary.failed, 0);
  ...
  const methodSamples = baseline.samples.filter((sample) => sample.caseId === "method-direct-en");
  assert.equal(methodSamples.length, 5);
  assert.equal(methodSamples.every((sample) => sample.passed && sample.selectedTools[0] === "trialbridge_method"), true);
