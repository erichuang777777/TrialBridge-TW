...
test("recorded stock Inspector evidence is two-of-six, metadata-only, and provider-policy bounded", async () => {
  ...
  assert.equal(artifact.result.checksPassed, 2);
  ...
  assert.deepEqual(artifact.result.checks.map((item) => item.status), ["pass", "pass", "not_run", "not_run", "not_run", "not_run"]);
