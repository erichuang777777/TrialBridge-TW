...
- UI, guided chat/matching, and WebMCP public search now share one persistent public Trial Index. Development uses SQLite; production can use PostgreSQL. TFDA and the complete ClinicalTrials.gov cancer corpus synchronize outside the request path, `/data-health` exposes source freshness and ingestion receipts, and exact displayed ClinicalTrials.gov records can be rechecked live before action. Patient notes never enter this index.
...
For the shared public index, run `npm run sync:trial-index -- --source=all`. The default SQLite database is ignored under `var/trial-index`; production sets `TRIAL_INDEX_BACKEND=postgres` and `DATABASE_URL`. A small, versioned NCI Thesaurus cancer-group snapshot is bundled under `data/public`; run `npm run sync:nci-terminology` to refresh it deliberately. The scheduled workflow requires the `TRIAL_INDEX_DATABASE_URL` repository secret and fails rather than pretending an ephemeral runner database is durable.
