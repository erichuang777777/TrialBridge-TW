...
- A second recorded run pinned the unmodified stock Model Context Tool Inspector at commit `f164a9aa5c3f6083f5976ccae308257bdf86cb99` / version 1.9.14. Its real background/content-script path discovered both public tools, parsed both schemas, manually executed the fixed empty-input method tool, and observed the fictional permission lifecycle as `2 → 2 → 6 → 2`: 3/6 Inspector cases passed. The two natural-language cases and cancellation/cleanup stay visibly not run; no Gemini key, model call, health information, raw schema, or execution result is stored in `evals/webmcp-inspector-extension-runtime.json`.
