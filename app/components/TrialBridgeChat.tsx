...
export function TrialBridgeChat({ initialSyntheticDemo = false }: { initialSyntheticDemo?: boolean }) {
  ...
  const [intakeMode, setIntakeMode] = useState<"agent" | "manual">(initialSyntheticDemo ? "manual" : "agent");
  ...
  return (
    <section className="chat-shell" id="private-chat" aria-labelledby="chat-title">
      <div className="chat-heading"><div><p className="eyebrow">Agentic TrialBridge</p><h2 id="chat-title">{state.language === "en" ? "Describe. Review. Compare." : "描述、確認、比較。"}</h2></div><div className="header-controls"><div className="header-language" role="group" aria-label="Language"><button aria-pressed={state.language === "en"} onClick={() => dispatch({ type: "SET_LANGUAGE", language: "en" })}>EN</button><button aria-pressed={state.language === "zh-Hant"} onClick={() => dispatch({ type: "SET_LANGUAGE", language: "zh-Hant" })}>繁中</button></div><div className="mode-toggle" role="group" aria-label={state.language === "en" ? "Workflow mode" : "工作模式"}><button aria-pressed={intakeMode === "agent"} onClick={() => setIntakeMode("agent")}>{state.language === "en" ? "Guided chat" : "對談引導"}</button><button aria-pressed={intakeMode === "manual"} onClick={() => setIntakeMode("manual")}>{state.language === "en" ? "Paste note" : "貼上病歷"}</button></div></div></div>
      <ol className="compact-progress" aria-label={state.language === "en" ? "Matching progress" : "配對進度"}>{[
        { number: 1, en: "Describe", zh: "描述" },
        { number: 2, en: "Review", zh: "確認" },
      ...
      <aside className="persistent-chat-panel" aria-labelledby="assistant-title">
        ...
        </form> : <div className="assistant-composer manual-assistant-state"><strong>{state.language === "en" ? "Paste-note mode" : "貼上病歷模式"}</strong><p>{state.language === "en" ? "Use the main panel to enter, extract, and confirm. Switch to Guided chat above whenever you want the agent to lead." : "請在主要工作區輸入、整理並確認；需要 Agent 引導時，可隨時切換為對談引導。"}</p></div>}
      </aside>
      </div>
    </section>
  );
