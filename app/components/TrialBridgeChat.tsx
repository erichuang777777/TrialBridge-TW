...
export function TrialBridgeChat() {
  ...
  const [intakeMode, setIntakeMode] = useState<"agent" | "manual" | null>(null);
  ...
  const step = useMemo(() => ({ role: 1, privacy: 1, capture: 1, mask_review: 1, extracting: 1, confirmation: 2, ready: 3 }[state.stage]), [state.stage]);
  ...
  async function sendAssistantMessage() {
    ...
    try {
      ...
      if (payload.workflowAction === "select_patient") dispatch({ type: "SET_SUBJECT_ROLE", role: "patient" });
      if (payload.workflowAction === "select_caregiver") dispatch({ type: "SET_SUBJECT_ROLE", role: "caregiver" });
  ...
  const assistantContext = state.stage === "role"
  ...
  return (
    <section className="chat-shell" id="private-chat" aria-labelledby="chat-title">
      ...
      {state.stage === "role" && <div className="chat-turn mode-onboarding"><div className="language-switch" aria-label="Language"><button className={state.language === "en" ? "selected" : ""} onClick={() => dispatch({ type: "SET_LANGUAGE", language: "en" })}>English</button><button className={state.language === "zh-Hant" ? "selected" : ""} onClick={() => dispatch({ type: "SET_LANGUAGE", language: "zh-Hant" })}>繁中</button></div><p className="eyebrow">{state.language === "en" ? "Choose how to work" : "選擇使用方式"}</p><h3>{state.language === "en" ? "Start with an agent or complete the steps yourself" : "由 Agent 引導，或自行完成每個步驟"}</h3><div className="mode-choice-grid"><button onClick={() => { setIntakeMode("agent"); dispatch({ type: "SELECT_ROLE", role: "patient" }); }}><strong>{state.language === "en" ? "Agent mode" : "代理模式"}</strong><small>{state.language === "en" ? "Chat is the main interface. The agent fills the shared note and guides every step." : "以對談為主要介面；Agent 填入共用病況並引導全部步驟。"}</small></button><button onClick={() => { setIntakeMode("manual"); dispatch({ type: "SELECT_ROLE", role: "patient" }); }}><strong>{state.language === "en" ? "Manual mode" : "手動模式"}</strong><small>{state.language === "en" ? "Enter the note, run extraction, review facts, and continue." : "自行輸入病歷、執行整理、核對 facts 後繼續。"}</small></button></div></div>}
