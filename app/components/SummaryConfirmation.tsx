...
export function SummaryConfirmation({ draft, maskResult, language, receipt, edits, checked, completed = false, onEdit, onCheck, onCheckAll, onBack, onFinish }: {
  ...
  return <div className="chat-turn compact-confirmation">
    ...
    {receipt && <section className="extraction-receipt" aria-labelledby="extraction-receipt-title"><div><strong id="extraction-receipt-title">{language === "en" ? "Cloud extraction receipt" : "雲端整理收據"}</strong><span>{language === "en" ? "localhost Ollama proxy → remote cloud" : "localhost Ollama proxy → 遠端雲端"}</span></div><dl><div><dt>{language === "en" ? "Requested" : "指定模型"}</dt><dd>{receipt.requestedModel}</dd></div><div><dt>{language === "en" ? "Provider reported" : "供應端回報"}</dt><dd>{receipt.reportedModel ?? (language === "en" ? "Not reported" : "未回報")}</dd></div><div><dt>{language === "en" ? "Elapsed" : "處理時間"}</dt><dd>{(receipt.latencyMs / 1_000).toFixed(1)}s</dd></div><div><dt>{language === "en" ? "TrialBridge storage" : "TrialBridge 儲存"}</dt><dd>{language === "en" ? "Not saved" : "未儲存"}</dd></div></dl><p>{language === "en" ? "The request used the masked note. Provider retention is not assessed." : "請求使用遮蔽後內容；尚未評估供應端保存政策。"}</p></section>}
