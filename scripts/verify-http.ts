...
const paths = ["/", "/trials", "/webmcp", "/method", "/privacy", "/robots.txt", "/sitemap.xml", "/manifest.webmanifest", "/opengraph-image", "/twitter-image", "/api/health"];
const results = [];
for (const path of paths) {
  const response = await fetch(`${baseUrl}${path}`);
  const contentType = response.headers.get("content-type") ?? "";
  const expectedType = path.endsWith(".txt") ? "text/plain" : path.endsWith(".xml") ? "application/xml" : path.endsWith(".webmanifest") ? "application/manifest+json" : path.includes("image") ? "image/png" : path.startsWith("/api/") ? "application/json" : "text/html";
