...
- Main product, database, and competition routes use the same branded Next.js loading surface. It names the destination, preserves an escape link home, uses one polite atomic status, reserves its final layout, shows no fake percentage or completion estimate, and stops its single decorative motion under `prefers-reduced-motion`.
...
- `/webmcp/quickstart` is the flat three-minute competition entrance; `/webmcp` is the technical appendix. Quickstart uses one headline, a four-metric summary, one live current-browser console, three equal next-step cards, and one appendix callout. It registers only the two public origin-scoped tools and permits only an explicit fixed no-input method execution. Each API, discovery, and header indicator succeeds independently rather than turning the whole row green from one state.
- Quickstart places one compact recorded Agentic Browsing evidence strip below the current-browser console. It must say recorded local audit, keep the current viewer separate, show text outcomes for schemas, accessibility tree, `llms.txt`, and CLS, and link to both the metadata and Chrome method. Use native `ul`/`li` semantics for repeated status checks; never override article roles to simulate a list.
...
- Recorded stock Inspector evidence is a separate compact partial-state banner immediately before the manual kit. It keeps the numeric pass count, passed capabilities, remaining checks, provider-policy boundary, and no-PHI statement visible in text; partial evidence uses a warning surface and can never be styled as an aggregate green pass.
