...
- The judge conformance matrix separates repository-verified checks, recorded model evaluation, and the remaining manual Inspector gate in every row. Its static downloadable JSON contains only capability metadata, evidence paths, source links, and artifact digests; it never reads browser-session or medical-workflow state and is labelled competition evidence rather than a protocol endpoint.
